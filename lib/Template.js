...
module.exports = {
    ...
    update_form_income: function(column) {
        ...
        var form = `
        <form action="/update_process/income?id=${column[0].id}" method="post">
            <p><input type="text" name="title" value="${column[0].title}"></p>
            <p><input type="text" name="price" value="${column[0].price}"></p>
    ...
    update_form_outcome: function(column) {
        var date = new Date(column[0].calen);
        var date_form = (date.getMonth()+1) + '/' + date.getDate() + '/' + date.getFullYear();
        var form = `
        <form action="/update_process/outcome?id=${column[0].id}" method="post">
            <p><input type="text" name="title" value="${column[0].title}"></p>
            <p><input type="text" name="target" value="${column[0].target}"></p>
            <p><input type="text" name="price" value="${column[0].price}"></p>
            <p><input type="text" name="date" id="datepicker" value="${date_form}"></p>
            <p><input type="submit" value="수정"></p>
        </form>
